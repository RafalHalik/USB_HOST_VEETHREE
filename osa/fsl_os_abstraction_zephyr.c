...
#include <string.h>
K_HEAP_DEFINE_NOCACHE(usb_heap, 8000);

void *OSA_MemoryAllocate(uint32_t length)
{
    // void *p = k_malloc(length);
    void *p = k_heap_alloc(&usb_heap, length, K_NO_WAIT);
    if (p != NULL)
    {
                           ...
                           osa_event_flags_t *pSetFlags)
{
    k_timeout_t timeout;
    uint32_t wakeflags;
    // printf("1 osa wait %d", k_uptime_get());
    ...
    // printf("2 osa wait %d", k_uptime_get());
    return KOSA_StatusSuccess;
}
