...
void USB_HostEhciTaskFunction(void *hostHandle)
{
    ...
    uint32_t bitSet;
    usb_device_handle deviceHandle;
    // usb_echo("doing nothing \r\n");
    if (hostHandle == NULL)
    {
        usb_echo("host invalid \r\n");
        return;
    }
    ehciInstance = (usb_host_ehci_instance_t *)((usb_host_instance_t *)hostHandle)->controllerHandle;
    // usb_echo("ehciInstance.deviceAttached %d \r\n", ehciInstance->deviceAttached);
    osa_status_t retval = OSA_EventWait(ehciInstance->taskEventHandle, 0xFF, 0, 1, &bitSet);
    if ( retval == KOSA_StatusSuccess) /* wait all event */
    {
